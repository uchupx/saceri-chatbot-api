...
func (r *EventRepoMongodb) GetAllEvents(ctx context.Context, keyword *string, limit, offset int) ([]models.EventModel, error) {
	collection := r.db.Database(databaseName).Collection(r.collectionName)
	...
	if keyword != nil && *keyword != "" {
		filter["$or"] = []bson.M{
			...
			{"place": bson.M{"$regex": *keyword, "$options": "i"}},
		}
	}
...
func (r *EventRepoMongodb) GetEventsByDates(ctx context.Context, dates []time.Time) ([]models.EventModel, error) {
	collection := r.db.Database(databaseName).Collection(r.collectionName)
